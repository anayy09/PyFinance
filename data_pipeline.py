...
def fetch_data(tickers):
    ...
    for ticker in tickers:
        data[ticker] = yf.download(ticker, start="2023-01-01", end="2023-12-31")
...
data_combined = pd.concat(data.values(), keys=data.keys(), names=['Ticker', 'Date'])
...
def remove_outliers(df, threshold=3):
    for column in df.select_dtypes(include=['float64', 'int64']).columns:
        ...
        df[column] = df_column.where(np.abs(z_scores) <= threshold, np.nan)  # Replace outliers with NaN
...
data_combined_cleaned = data_combined.groupby(level=0, group_keys=False).apply(remove_outliers)

# Ensuring Timestamp Consistency without using 'inplace'
data_combined_cleaned.index = data_combined_cleaned.index.set_levels(pd.to_datetime(data_combined_cleaned.index.levels[1]), level=1)
...
# After processing your data and having `data_transformed` ready
create_database('stock_data.db')  # Ensure the database & table exists
insert_data(data_transformed)  # Insert the transformed data into the database